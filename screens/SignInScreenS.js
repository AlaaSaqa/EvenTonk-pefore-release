...
function submitData (){
   var v =0;
        for(let i =0;i<datac.length;i++){
            const obj = datac[i];
            if (obj.number==number&&password==obj.password){
                  v=1;
                  navigation.replace('Home',datac[i]) ;
            }
        }
    ...
    const loginHandle = (userName, password) => {
        ...
        if ( data.username.length == 0 || data.password.length == 0 ) {
            Alert.alert('Wrong Input!', 'Registration number or password field cannot be empty.', [
                {text: 'Okay'}
            ]);
            return;
                }
    ...
    return (
      <View style={styles.container}>
        ...
        >
            ...
            <Text style={styles.errorMsg}>Registration number must be 4 characters long.</Text>
            </Animatable.View>
            }
