...
const commId= route.params.student; //myself 
const comm = route.params.item; //who i vist his page
   ...
   */
   ...
   function handleButtonPress(){
    ...
    fetch("http://10.0.2.2:4000/signInS")
          ...
          for (let i =0;i<results.length;i++){
           // console.log(commId)
            ...
            if (results[i]._id==commId)
            {
             ...
             if (results[i].number>comm.number)
             {
               max = results[i].number;
               min = comm.number;
             }
             else
              {
       ...
       //    navigation.navigate('conversation', { thread: item })
  ...
  function foll(){
    ...
    function renderFollowers() {
      ...
      for (let i=0;i<DFollowers.length;i++){
        if (DFollowers[i].studentId==comm._id){
      ...
      const renderItem = ({ item }) => {
          return(
            ...
            onPress={() => navigation.replace("commPage",  {userId:commId,item:item})}
        >
            {/* Image */}
...
<View
                    ...
                    >
                        <Text style={{ ...FONTS.h4 , color:COLORS.white ,flexWrap:"wrap",margin:15,}}>{item.name}</Text>
                    </View>
