...
const commId= route.params.userId;// student as senderv in mongo
const comm = route.params.item; //reciver as community
...
useEffect(()=>{
  ...
  fetch("http://10.0.2.2:4000/likedComm")
  .then(res=>res.json())
  .then(results=>{
    setLikes(results) ;
});

fetch("http://10.0.2.2:4000/commPost")
.then(res=>res.json())
.then(results=>{
    setDPosts(results)
});
...
const handleLikeRed=(item)=>{ 
        ...
        .then(res=>{
          ...
          for (let i = 0 ;i<res.length;i++){
            if (res[i].student==commId){
              var s=res[i].posts+"+"+item._id;
              fetch("http://10.0.2.2:4000/update-likedCommPosts",{
            method :"post",
            headers:{
  ...
  const onPressFollow= () => {
    if (follow=='Follow'){
        ...
        .then(res=>{
          ...
          for (let i = 0 ;i<res.length;i++){
            if (res[i].studentId==commId){
              var s=res[i].commIds+"+"+comm._id;
              fetch("http://10.0.2.2:4000/update-studentComms",{
            method :"post",
            headers:{
                'Content-Type':'application/json'
            },
        ...
        <TouchableOpacity
            ...
            <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' , flexDirection:'row',marginRight:'2%'}}>
                    ...
                    >
                        ...
                        onPress={()=>{ 
                        navigation.replace('search',{item :commId,s:search,type:'student'})}}
                        >
                        <Image
                            source={icons.search}
                            resizeMode="contain"
                   ...
                   <TouchableOpacity
                    onPress={()=>{
                      for (let i =0;i<posts.length;i++){
                        if (item._id==posts[i]._id){
                            likes[i].likes=0;
                        }
                      }
                      handleLikeRed(item);
                      handleLikeWhite(item); }}
                   >
                  <Image
                          source={icons.like}
                          style={{
    ...
    function renderFollowers() {
      ...
      const renderItem = ({ item }) => (
       ...
       onPress={() => 
        ...
        navigation.replace("conversation",{
           max:(parseInt(st.number)>parseInt(item.number)? st.number: item.number),
           min:(parseInt(st.number)<parseInt(item.number)? st.number: item.number),
          sender:st.number,
          reciver:item.number
         })
        }
      ...
      return (
       <View>
        ...
        }}>  {((follow=='Followed')&&(students.length+1))||(students.length)} following people </Text>
        <FlatList  
              data={students}
              keyExtractor={item => `${item.id}`}
              renderItem={renderItem}
      ...
      for (let i=0;i<DEvents.length;i++){
        ...
        const renderItem = ({ item }) => (
            ...
            <TouchableOpacity
                ...
                onPress={() => navigation.navigate("EventShowScreen",{event:item,userId:commId} 
                  )}
            >
